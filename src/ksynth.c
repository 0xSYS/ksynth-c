...
struct Voice** int_allocate_voices(struct KSynth* ksynth_instance) {
	if(ksynth_instance->max_polyphony < 1) ksynth_instance->max_polyphony = 1;
...
void int_free_samples(struct Sample** samples, unsigned long count) {
	if(samples) {
		if(count > 0) {
			for(unsigned long i = 0; i < count; i++) {
				if(samples[i]) {
					if(samples[i]->audio_data) free(samples[i]->audio_data);
			...
			if(!voice->killed) {
				...
				for(unsigned int j = 0; j < buffer_size / num_channels; ++j) {
					...
					if(!voice->killed && (voice->tokill || natural_decay)) {
						...
						if((!ksynth_instance->channels[voice->channel].sustain && voice->tokill) || natural_decay) voice->curfalloff++;
